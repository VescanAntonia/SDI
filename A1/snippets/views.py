...
class AddCitybreaks(APIView):
    def post(self,request,id):
        parent = CityBreak.query.get_or_404(id)

        # Get the child data from the request body
        child_data = request.json.get('person')

        # Validate the child data
        if not isinstance(child_data, list):
            return Response({'error': 'Invalid input data'})

        # Create the child objects and associate them with the parent
        children = []
        for child in child_data:
            child_obj = Person(**child)
            child_obj.parent_id = id
            children.append(child_obj)
        
        # Add the child objects to the database
        # db.session.add_all(children)
        # db.session.commit()

        # Return a success response
        return Response({'success': 'Children added successfully'})
